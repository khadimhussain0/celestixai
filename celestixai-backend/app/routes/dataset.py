...
def create_dataset(dataset: UploadFile = File(...), db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    ...
    dataset_db = Dataset(owner_id=current_user.id, uuid=dataset_uuid, filename=dataset.filename, file_size=os.path.getsize(dataset_path))
    db.add(dataset_db)
    db.commit()
